...
class AmapService:
    ...
    def search_poi(self, keywords: str, city: str, citylimit: bool = True) -> List[POIInfo]:
        ...
        """
        try:
            cache_key = self.cache.build_key(keywords, city, citylimit)
            result = self.cache.get_or_set(
                "amap_poi",
                cache_key,
                lambda: self.mcp_tool.run({
                    "action": "call_tool",
    ...
    def get_weather(self, city: str) -> List[WeatherInfo]:
        ...
        """
        try:
            cache_key = self.cache.build_key(city)
            result = self.cache.get_or_set(
                "amap_weather",
                cache_key,
                lambda: self.mcp_tool.run({
    ...
    def plan_route(
        ...
        try:
            ...
            cache_key = self.cache.build_key(tool_name, arguments)
            result = self.cache.get_or_set(
                "amap_route",
                cache_key,
                lambda: self.mcp_tool.run({
    ...
    def geocode(self, address: str, city: Optional[str] = None) -> Optional[Location]:
        ...
        try:
            ...
            cache_key = self.cache.build_key(arguments)
            result = self.cache.get_or_set(
                "amap_geocode",
                cache_key,
                lambda: self.mcp_tool.run({
    ...
    def get_poi_detail(self, poi_id: str) -> Dict[str, Any]:
        ...
        """
        try:
            cache_key = self.cache.build_key(poi_id)
            result = self.cache.get_or_set(
                "amap_poi_detail",
                cache_key,
                lambda: self.mcp_tool.run({
